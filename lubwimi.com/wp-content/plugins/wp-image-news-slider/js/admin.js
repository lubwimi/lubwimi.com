...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ slider_upload_dir+"/"+file.name + '&task=slider_resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
...
function on_queue_completed(file)
{
	...
	slider_refresh_images_table(slider_current_album_id);
...
function slider_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task=slider_get_albums_table', function(res)
	{
		slider_set_data_tables();
		if (res.status == 'ok') 
		{
...
function slider_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task=slider_get_albums_images_table&album_id='+album_id, function(res)
	{
		slider_set_data_tables();
		if (res.status == 'ok') 
		{
...
function re_order()
{
	...
	if( task == 'slider_reorder_album' )
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
	if( task == 'slider_reorder_image' )
	...
	jQuery.get('index.php?' + params, function(res)
	{
		if( task == 'slider_reorder_image' )
		{
			slider_refresh_images_table(album_id);
		}
		if( task == 'slider_reorder_album' )
			slider_refresh_albums_table();
...
var tpager = null;
var ipager = null;
function slider_set_data_tables()
{
	//add event to bulk actions
	jQuery('.bulk_form').live('submit', slider_submit_bulk);
...
function slider_submit_bulk()
{
	var task = this.task.value;
	var album_id = null;
	if( task == 'slider_bulk_delete_albums' || task == 'slider_bulk_enable_albums' || task == 'slider_bulk_disable_albums')
	{
	...
	else if( task == 'slider_bulk_delete_images' || task == 'slider_bulk_enable_images' || task == 'slider_bulk_disable_images' )
	{
...
{
	//preload images
	loading.src = slider_url + '/images/loading.gif';
...
/*
		var ops = {
					'uploader'		: slider_url + '/js/uploadify-2.1.4/uploadify.swf',
					'script'		: slider_url + '/js/uploadify-2.1.4/uploadify.php',
					'cancelImg' 	: slider_url + '/js/uploadify-2.1.4/cancel.png',
				    'folder'    	: slider_upload_dir,
				    'auto'			: true,
					'multi'			: true,
					'fileExt'        : '*.jpg;*.gif;*.png',
...
*/
		var swfu;
			...
			var settings = {
				flash_url : slider_url+"/js/swfupload/js/swfupload.swf",
				upload_url: slider_url+"/js/swfupload/js/upload.php",
				post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","folder":slider_upload_dir},
				...
				// Button settings
				button_image_url: slider_url+"/js/swfupload/js/TestImageNoText_65x29.png",
				button_width: "130",
				button_height: "29",
	...
	jQuery('.slider_delete_album').live('click', slider_delete_album);
	...
	jQuery('table#album_images .enable, table#albums_table .enable').live('click', slider_enable);
	jQuery('table#album_images .disable, table#albums_table .disable').live('click', slider_disable);
